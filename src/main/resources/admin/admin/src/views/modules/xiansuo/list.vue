        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '线索等级' : ''">
                        <el-select v-model="searchForm.xiansuoDengjiTypes" placeholder="请选择线索等级">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in xiansuoDengjiTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xiansuo','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoshouxiangmuliucheng/upload/xiansuoMuBan.xls"
                        >批量导入线索数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xiansuo','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xiansuoUploadSuccess"
                                :on-error="xiansuoUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xiansuo','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xiansuo.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="发现时间">
                        <template slot-scope="scope">
                            {{scope.row.xiansuoTime}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiansuoDengjiTypes"
                                      header-align="center"
                                      label="线索等级">
                        <template slot-scope="scope">
                            {{scope.row.xiansuoDengjiValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiansuoTypes"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="xiansuoFile"
                                     header-align="center"
                                     width="200"
                                     label="附件">
                        <template slot-scope="scope">
                            <div v-if="scope.row.xiansuoFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.xiansuoFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xiansuo','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xiansuo','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xiansuo','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                yonghuId : null,
                xiansuoUuidNumber : null,
                xiansuoName : null,
                xiansuoAddress : null,
                xiansuoTime : null,
                xiansuoDengjiTypes : null,
                xiansuoTypes : null,
                xiansuoFile : null,
                xiansuoContent : null,
                insertTime : null,
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'xiansuo',//当前表表名,
                        sumColum : 'xiansuo_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xiansuo_name',//分组字符串字段
                        // types : 'xiansuo_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['xiansuoDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            xiansuoUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xiansuo/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
