        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('zhaobiao','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoshouxiangmuliucheng/upload/zhaobiaoMuBan.xls"
                        >批量导入招标数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('zhaobiao','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="zhaobiaoUploadSuccess"
                                :on-error="zhaobiaoUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('zhaobiao','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "zhaobiao.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="qiyePhoto"
                        ...
                        <template slot-scope="scope">
                            <div v-if="scope.row.qiyePhoto">
                                <img :src="$base.url+scope.row.qiyePhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="zhaobiaoAddress"
                                   header-align="center"
                                   label="招标地点">
                        <template slot-scope="scope">
                            {{scope.row.zhaobiaoAddress}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="zhaobiaoFile"
                                     header-align="center"
                                     width="200"
                                     label="附件">
                        <template slot-scope="scope">
                            <div v-if="scope.row.zhaobiaoFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.zhaobiaoFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('zhaobiao','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('zhaobiao','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('zhaobiao','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                qiyeId : null,
                zhaobiaoUuidNumber : null,
                zhaobiaoName : null,
                zhaobiaoAddress : null,
                zhaobiaoTime : null,
                zhaobiaoTypes : null,
                zhaobiaoFile : null,
                zhaobiaoContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'zhaobiao',//当前表表名,
                        sumColum : 'zhaobiao_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'zhaobiao_name',//分组字符串字段
                        // types : 'zhaobiao_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['zhaobiaoDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            zhaobiaoUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "zhaobiao/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
